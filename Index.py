...
# Makes a dictionary : key= word (e.g. "hello"); value= list of list [[DOCN0., no of times hello occured],[],[]...]
...
def main():
	...
	global title_doc
	global File_Count
	#f6 = open("title_index","w")
	file = 0



	for event, elem in context:
		tag = elem.tag # tag= title.metadata

		if event == 'end': # to reach "</title>"
			given_tag = processing(str(tag)) # given_tag= "title"

		#********************************************************************
		# Check for Page

			if given_tag == 'title':
				Title_Extraction(elem.text) # extract data
				DOC_NO += 1
